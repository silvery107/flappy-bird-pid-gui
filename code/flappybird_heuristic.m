function flappybird_heuristic(app)
...
GAME.FRAME_DURATION = [];
GAME.GRAVITY = 0.15;%0.1356; %0.15; %0.2; %1356;  % empirical gravity constant
...
while 1
    initGame();
    app.isStart = true;
    GAME.FRAME_DURATION = 1/(GAME.N_UPDATE_PERSEC*app.GameSpeed);
    CurrentFrameNo = double(0);
    collide = false;
    ...
    while 1
        ...
        while (curTime >= ((CurrentFrameNo) * GAME.FRAME_DURATION) && loops < GAME.MAX_FRAME_SKIP)
            ...
            if strcmp(app.ControlModeSwitch.Value,'Auto')
                FlyKeyStatus = true;
            end
            app.RestartKey = false;
            if FlyKeyStatus  % If left key is pressed
                if ~gameover
                    if strcmp(app.ControlModeSwitch.Value,'Auto')
                        BirdPos = 176 - Bird.ScreenPos(2);
                        TubePos = setPointY + app.yOffset;
                        if app.CheckBox.Value == 0
                            ...
                            % PID controller
                            err = BirdPos - TubePos;
                            app.IntegralErr = app.IntegralErr + err;
                            PIDresult = app.Kp*err + app.Ki*app.IntegralErr + app.Kd*(err-app.LastErr);
                            app.LastErr = err;
                            % only allow compensating upwards
                            if PIDresult < 0
                                Bird.SpeedY = max(-2.5,PIDresult);
                            end
                        end
                    else
                        Bird.SpeedY = -2.5;
            ...
            if Flags.PreGame
                ...
                if ~gameover
                    scrollTubes(1);
                    % refresh the index of the front tube
                    if Tubes.ScreenX(Tubes.FrontP) + app.xOffset < Bird.ScreenPos(1)
            ...
            % real time plot begin
            plotFPS = GAME.N_UPDATE_PERSEC*app.GameSpeed / 12;
            if (app.status) && strcmp(app.TrajectoryPlotSwitch.Value,'On') && ~mod(CurrentFrameNo,plotFPS) && ~collide && ~fall_to_bottom
                x0 = Tubes.ScreenX(setPointX);
                y0 = Tubes.VOffset(setPointX);
                x1 = Tubes.ScreenX(mod(setPointX,3)+1);
                y1 = Tubes.VOffset(mod(setPointX,3)+1);
                yb = 176 - Bird.ScreenPos(2);
                plot(app.UIAxes,[x0,x0,x0+TUBE.WIDTH,x0+TUBE.WIDTH,x1,x1,x1+TUBE.WIDTH,x1+TUBE.WIDTH],[-2,y0,y0,-2,-2,y1,y1,-2],'g');
                hold(app.UIAxes,'on');
                plot(app.UIAxes,[x0,x0,x0+TUBE.WIDTH,x0+TUBE.WIDTH,x1,x1,x1+TUBE.WIDTH,x1+TUBE.WIDTH],[202,y0+TUBE.V_SPACE,y0+TUBE.V_SPACE,202,202,y1+TUBE.V_SPACE,y1+TUBE.V_SPACE,200],'g');
                plot(app.UIAxes,45,yb,'or');
                plot(app.UIAxes,[45,45,45+app.xOffset,45],[y0,y0+app.yOffset,y0+app.yOffset,y0],'b');
                plot(app.UIAxes,[45,45,45+app.xOffset],[y0,y0+app.yOffset,y0+app.yOffset],'+b');
                hold(app.UIAxes,'off');
            end
            % real time plot end
            if  fall_to_bottom
                app.GameSpeedSlider.Enable = true;
            else
                app.GameSpeedSlider.Enable = false;
            end
        end
        ...
        if fall_to_bottom
            ...
            save sprites2.mat Best -append
            if app.RestartKey == true
                app.RestartKey = false;
                FlyKeyStatus = true;
                app.ControlModeSwitch.Value = 'Manual';
                app.TrajectoryPlotSwitch.Value = 'Off';
                GAME.FRAME_DURATION = 1/(GAME.N_UPDATE_PERSEC*app.GameSpeed);% refresh the game speed
            end
            if FlyKeyStatus
                FlyKeyStatus = false;
                break;
